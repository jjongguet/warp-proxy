...
`metadata.warp_previous_response_id`에 이전 응답의 `id`를 넣으면, 프록시가 내부 conversation store에서 Oz conversation id를 조회하여 `--conversation-id`로 전달한다.
...
- **`WARP_PROXY_CONVERSATION_STORE`** — 대화 저장 경로 (기본 `~/.warptocli/conversations.json`)
...
## 5. Non-streaming success response
...
```json
{
  "object": "list",
  "data": [
    {"id": "warp-oz-cli", "object": "model", "owned_by": "warptocli"},
    {"id": "warp-oz-cli/claude-3.5-sonnet", "object": "model", "owned_by": "warptocli"}
  ]
}
```
